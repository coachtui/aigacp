...
export default function MxWorkOrdersPage() {
  ...
  const sorted = [...visible].sort((a, b) => {
    // OPS-blocking first, then priority, then date
    ...
    <PageContainer maxWidth="wide">
      ...
      {createdId && (
        <div className="mb-4 flex items-center gap-2 px-4 py-3 bg-teal/10 border border-teal/30 rounded-lg text-sm text-teal">
          <Wrench size={14} />
          Work order created.{" "}
          <Link href={`/modules/mx/work-orders/${createdId}`} className="underline font-semibold">
            View details →
          </Link>
        </div>
      )}
          ...
          {sorted.map((wo) => {
            ...
            return (
              <Link
                ...
                className={`grid grid-cols-12 gap-2 items-center px-4 py-3 border rounded-[var(--radius-card)] hover:border-teal/30 transition-colors ${
                  urgency === "overdue"
